pro ptrack_field,pn,frame, global_v = global_v, local_ue = local_ue, epar = epar, rho = rho, eta = eta, energy = energy
...
;lare normalisations
lscl =    10.000000000000000     
bscl =   2.99999999999999989E-002
rhoscl =   1.66999999999999995E-012
vscl =    20708950.362051707     
P0scl =    716.19724391352895     
tscl =   4.82882996249031825E-007
jscl =    2387.3241463784298     
Escl =    621268.51086155116     
epsscl =    428860625097921.50     
tempscl =    62346481234.325447     
...
IF CURRENT THEN BEGIN
 ...
 pl1 = plot(p.x/lscl,p.y/lscl,/overplot,vert_colors = p.t/max(p.t)*255, rgb_table = 11,thick = 3)
...
if keyword_set(local_ue) then begin
 v2 = vector(reform(p.UE(0,1:p_len - 1:vec_skip)),reform(p.UE(1,1:p_len - 1:vec_skip)),p.x(1:p_len - 1:vec_skip)/lscl,p.y(1:p_len - 1:vec_skip)/lscl,data_location = 0,/overplot)
 v3 = vector(reform(p.UE(0,1:p_len - 1:vec_skip)),reform(p.UE(1,1:p_len - 1:vec_skip)),p.x(1:p_len - 1:vec_skip)/lscl,p.y(1:p_len - 1:vec_skip)/lscl,data_location = 0, position = [0.13,0.1,0.9,0.87],xtitle = '$x$', ytitle = '$y$')
 pl1 = plot(p.x/lscl,p.y/lscl,/overplot,vert_colors = p.t/max(p.t)*255, rgb_table = 11,thick = 3)
 c1 = colorbar(range = [min(p.t),max(p.t)],position = [0.13,0.95,0.9,0.98],title = 'time',rgb_table = 11)
 ue = sqrt(p.ue(0,*)^2 + p.ue(1,*)^2 + p.ue(2,*)^2)
 pl2 = plot(p.x/lscl,p.y/lscl,vert_colors = ue/max(ue)*255, rgb_table = 11,thick = 3)
...
if keyword_set(epar) then begin
 pl2 = plot(p.x/lscl,p.y/lscl,vert_colors = bytscl(p.epar), rgb_table = 11,thick = 3,position = [0.13,0.1,0.9,0.87],xtitle = '$x$', ytitle = '$y$')
 c2 = colorbar(range = [min(p.epar),max(p.epar)],position = [0.13,0.95,0.9,0.98],title = 'E parallel (V/m)')
endif
if keyword_set(rho) then begin
 pl2 = plot(p.x/lscl,p.y/lscl,vert_colors = bytscl(p.rho), rgb_table = 11,thick = 3,position = [0.13,0.1,0.9,0.87],xtitle = '$x$', ytitle = '$y$')
 c2 = colorbar(range = [min(p.rho),max(p.rho)],position = [0.13,0.95,0.9,0.98],title = 'rho')
endif
if keyword_set(eta) then begin
 pl2 = plot(p.x/lscl,p.y/lscl,vert_colors = bytscl(p.eta), rgb_table = 11,thick = 3,position = [0.13,0.1,0.9,0.87],xtitle = '$x$', ytitle = '$y$')
...
if keyword_set(energy) then begin
 ke_norm = p.ke*tscl/(lscl*lscl*bscl)
 pl2 = plot(p.x/lscl,p.y/lscl,vert_colors = bytscl(p.ke/1.0E3), rgb_table = 11,thick = 3,position = [0.13,0.1,0.9,0.87],xtitle = '$x$', ytitle = '$y$')
 c2 = colorbar(range = [min(p.ke/1.0E3),max(p.ke/1.0E3)],position = [0.13,0.95,0.9,0.98],title = 'Energy (keV)')
endif
